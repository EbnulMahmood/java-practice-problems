...
public class State implements Serializable {
    ...
    public static void initializeProblem(int dimension) {
        n = dimension;// Write the code here
    }
    ...
    public State get_best_successor() {
        ...
        int bestVal = Integer.MAX_VALUE;// Assign bestVal with Integer.MAX_VALUE
        ...
        for (int queenNumber = 0; queenNumber < n; ++queenNumber) {
            for (int newRow = 0; newRow < n; ++newRow) {
                ...
                if (bestVal > successor.objective_function()) {
                    bestSuccessor = successor;// Assign successor to best successor
                    bestVal = successor.objective_function();// Assign successors objective value to bestval using objective function
                }
            }
        }
    ...
    public static State get_random_state() {
        ...
        for (int i = 0; i < n; ++i) {
            state.r[i] = random.nextInt(n);// Assign state.r[i] with random.nextInt(n)
        }
